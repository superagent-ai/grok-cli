...
const MemoizedChatEntry = React.memo(({ entry, index }: { entry: ChatEntry; index: number }) => {
  const renderDiff = (diffContent: string, filename?: string) => {
    return <DiffRenderer diffContent={diffContent} filename={filename} terminalWidth={80} />;
  };
      ...
      return (
        <Box key={index} flexDirection="column" marginTop={1}>
          <Box>
            ...
            <Text color="white"> {filePath ? `${actionName}(${filePath})` : actionName}</Text>
          </Box>
...
export function ChatHistory({ entries, isConfirmationActive = false }: ChatHistoryProps) {
